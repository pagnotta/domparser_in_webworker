...
For the code in `worker.js` I use the example from the [xmldom GitHub page](https://github.com/jindw/xmldom#example). I ignore the fact that this code would not run in a webworker because it contains a `require()`. Browserify will transform it later to code that runs in the webworker.
